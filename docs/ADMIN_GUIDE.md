...
The live catalog determines available model IDs. Missing capability fields can use exact matches in AgentRouter's models.dev registry; unknown tools/context and models below 256k stay excluded. Catalog visibility, promotional credits and saving a key do not prove usable inference or free billing. Inference must succeed before the route is shown as verified healthy. Saving a key performs a read-only catalog check and leaves inference unverified; no billable request is made on save. The adapter leaves reasoning controls at the provider default because a shared effort parameter is not documented.

**If AgentRouter reports `unauthorized_client_error`:** the provider rejected this gateway client. A non-expiring key or available credits do not establish client authorization. Ask AgentRouter support to authorize FCC or confirm a supported gateway endpoint for your account. Providers and Routing controls show this specific cause, and automatic routing excludes the rejected catalog while considering other eligible providers. Changing the key is not prescribed for this response. Similar client restrictions have been [reported in AgentRouter's own issue tracker](https://github.com/agentrouter-org/docs/issues/21); an adapter implementation alone does not establish upstream permission. Refresh catalogs after AgentRouter restores access.
