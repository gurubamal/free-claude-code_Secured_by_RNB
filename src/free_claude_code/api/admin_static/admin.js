...
async function api(path, options = {}) {
  const response = await fetch(path, {
    ...options,
    headers: { ...(options.headers || {}), "Content-Type": "application/json", "X-FCC-Admin": "1" },
    cache: "no-store",
  });
  if (!response.ok) {
...
async function waitForRestart(restart, target) {
  ...
  const statusUrl = new URL("/admin/ready", target);
...
async function reconnectAfterRestart() {
  ...
  try {
    ...
    window.location.replace(new URL("/admin/login", target).href);
  } catch (error) {
...
byId("hardenedLogout").addEventListener("click", async () => {
