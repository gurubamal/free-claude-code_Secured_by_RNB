...
class Settings(BaseModel):
    ...
    model: NonEmptyString = Field(
        default="open_router/openrouter/free",
    ...
    def require_loopback_bind(cls, value: str) -> str:
        ...
        try:
            ...
            pass
        raise ValueError("This hardened build binds only to a loopback IP address")

    @field_validator("proxy_auth_enabled")
    @classmethod
    def require_authentication(cls, value: bool) -> bool:
        if not value:
            raise ValueError("Proxy authentication cannot be disabled in this build")
        return value

    @field_validator("proxy_auth_token")
    @classmethod
    def require_strong_proxy_token(cls, value: str) -> str:
        if len(value) < 32 or len(set(value)) < 12:
            raise ValueError("Use a generated proxy token with at least 32 characters")
        return value

    @model_validator(mode="after")
    def require_remote_sender_and_provider_boundaries(self) -> Settings:
        if (
            self.messaging_platform == "telegram"
            and self.telegram_bot_token
            and (
                not self.allowed_telegram_user_id
                or not self.allowed_telegram_user_id.isdecimal()
            )
        ):
            raise ValueError("Telegram requires an explicit numeric allowed user ID")
        if (
            self.messaging_platform == "discord"
            and self.discord_bot_token
            and not self.allowed_discord_channels
        ):
            raise ValueError("Discord requires explicit allowed channel IDs")
        if self.messaging_platform != "none" and not self.allowed_dir:
            raise ValueError("Messaging requires an explicit workspace directory")
        return self
