...
def consolidate_managed_config(
    ...
    # Fresh fork state only: do not silently import another application's keys.
    base_path: Path | None = managed if managed.exists() else None
    ...
    if base_is_managed:
        ...
        if schema == CONFIG_SCHEMA_VERSION:
            ...
            _materialize_auth_state(normalized, process, had_legacy_state=True)
    ...
    explicit_path = None  # Configure this fork explicitly through its own Admin UI.
...
def atomic_write_managed_config(
    ...
    *,
    ...
    atomic_write_private_text(target, rendered)
    return rendered
...
def _materialize_auth_state(
    ...
    *,
    ...
    values.setdefault(PROXY_AUTH_ENABLED_ENV, "true")
    if (
        not values.get(ANTHROPIC_AUTH_TOKEN_ENV)
        or values[ANTHROPIC_AUTH_TOKEN_ENV] == "freecc"
    ):
        values[ANTHROPIC_AUTH_TOKEN_ENV] = secrets.token_urlsafe(32)
