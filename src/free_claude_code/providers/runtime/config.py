...
def build_provider_config(
    ...
    if (
        ...
        raise ApplicationUnavailableError(
            "Provider URL must have a host and no embedded credentials, query or fragment"
        )
    local_host = parsed.hostname.lower() == "localhost"
    with suppress(ValueError):
        local_host = local_host or ipaddress.ip_address(parsed.hostname).is_loopback
    if parsed.scheme != "https" and not (parsed.scheme == "http" and local_host):
        raise ApplicationUnavailableError(
            "Provider URLs require HTTPS, except loopback local models"
        )
