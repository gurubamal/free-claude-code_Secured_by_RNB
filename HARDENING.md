...
OpenRouter's router selects among available free models while filtering for required features. Providers can still impose quotas or be unavailable. Requests are retried only within bounded policies; the Chat adapter allows at most three attempts for selected pre-output failures. Recognized daily free-quota exhaustion stops immediate retries and reports the supplied reset time. The Messages/Responses provider also honors that terminal limit after accepting stream output. A delivered stream/tool call is never silently restarted by the Chat adapter. Exhaustion produces an error rather than enabling paid models. A new client request can still contact the provider; there is no persistent quota cache or automatic task queue.
