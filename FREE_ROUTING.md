...
Policy updated 2026-09-21. Required context is **at least 256,000 tokens** (decimal 256k, inclusive) for every primary and fallback, including paid, subscription and local models. An experimental maximum does not replace the provider's active default limit. Within each billing and health tier, models above 512,000 tokens remain preferred; a saved manual choice takes precedence when eligible. Context size is not a daily allowance, output allowance or evidence of coding quality.
...
Max does not enlarge the current 8,192-token automatic output ceiling or consume the whole context window as reasoning. Reasoning and visible output can share that ceiling. Provider quotas, paid permissions, the 256k floor, tool support and request-fit checks still apply. A higher effort request is not proof of a particular amount of internal computation. The shipped root default remains **Client**; Max is an explicit saved preference.
...
5. Refresh catalogs and review eligibility. Known tool support, at least 256k context and request fit are required. A valid account may still have no eligible or available models.
...
256k minimum checks. Missing preferred models do not require a manual switch.
...
| --- | --- | --- |
| AgentRouter | Paid-API permission required; `https://agentrouter.org/v1`; authenticated `/models` discovery and OpenAI-compatible Chat Completions. Live IDs must have known tools and context of at least 256k; exact AgentRouter registry metadata can fill missing fields. A `401 unauthorized_client_error` means gateway client access was rejected: the catalog stays excluded until access is restored. | [Official compatibility guide](https://github.com/agentrouter-org/docs/blob/main/en/qwencode.md), [access troubleshooting](docs/ADMIN_GUIDE.md#agentrouter) |
...
| Atria | Paid-API permission required; `https://api.atria-asi.ai/v1`; exact `Atria-Dawn-Preview` ID. Documented 256k context meets the floor, subject to request fit and account capacity. | [Atria API](https://api.atria-asi.ai/docs) |
| Inception | Paid-API permission required; `https://api.inceptionlabs.ai/v1`; chat-only discovery at `/chat/completions/models`. Mercury 2.5 (260k) meets the floor; Mercury 2 (128k) remains excluded. | [Models](https://docs.inceptionlabs.ai/get-started/models), [chat catalog](https://docs.inceptionlabs.ai/api-reference/models/list-chat-models) |
| Gemini / Google account | Paid-API opt-in; your own Desktop OAuth client and Cloud project; Messages/Responses/Chat, renewable browser login; same 256k floor | [OAuth setup](docs/GOOGLE_ACCOUNT.md), [Google OAuth](https://ai.google.dev/gemini-api/docs/oauth) |
...
| OpenAI / ChatGPT | Connected subscription; Messages/Responses only; primary default context must be at least 256k | [Authentication](https://learn.chatgpt.com/docs/auth) |
...
These additional API-key integrations use the paid switch even when a plan includes credits; connecting these accounts does not establish free capacity. Cline's catalog may omit capabilities, requiring matching registry metadata. Cline and Command Code can use exact model-ID matches from the OpenRouter registry for missing capabilities and limits; primary gateway context takes precedence. AgentRouter uses only its own registry namespace and live catalog intersection, with no model-name guessing or hardcoded availability. Its adapter does not inject a reasoning-effort field because the public guide does not establish a shared control across models. These integrations do not install the providers' native coding harnesses.

Atria and Inception were reviewed on 2026-09-20. Atria supplements only the exact live `Atria-Dawn-Preview` catalog row with primary documented limits (256,000 context / 65,536 output), text input and tool support when missing; unknown versions receive no inferred limits. Inception reads primary `context_length`, `max_output_length`, `input_modalities` and `supported_features` from its Chat catalog, excluding FIM/edit-only models by using the dedicated endpoint. Its adapter uses standard append-only SSE, not diffusion visualization, and maps reasoning to the documented `instant`/`low`/`medium`/`high` values. Dawn Preview and Mercury 2.5 meet the 256k floor adopted on 2026-09-21; smaller or unknown limits remain excluded. Future live entries must independently meet the same checks. The original integration review did not include authenticated inference; a loaded catalog alone still does not prove current capacity.
...
- Request fit: conservative UTF-8 byte estimate for readable input plus capped output; no history truncation. Known reasoning replay envelopes are estimated from their readable content, without counting duplicate encoded signatures as text. Each image reserves 16,384 tokens instead of counting its base64 representation. Opaque-only or malformed replay retains its original estimate. Tool arguments, schemas and unknown content stay literal. These are approximate admission checks; a provider tokenizer can still reject a request. Such a failure is handled before output when possible.
- If the estimated request exceeds every compatible enabled model in the current catalog, return a context-size error immediately instead of waiting for quota. Cooling-down routes that can fit still use normal automatic recovery. Compaction sends conversation history too, so an inflated size estimate can otherwise keep a compaction request waiting despite an available smaller route.
- Candidate order: saved manual choice first when eligible, then saved billing categories. Within each category, recently verified routes precede untested/stale routes and previously failed routes due for recheck. Within each health tier, models above 512k precede the 256k–512k fallback tier; within each context tier, free-family order is **DeepSeek V4.1 Flash → Kimi K3 → Qwen 3.8 Max → GLM 5.3 Flash**, across providers, then other eligible free models. Provider order and last success break remaining ties. Cooldown expiry permits a recheck; it does not prove recovery. This is a configured availability policy, not a benchmarked ranking.
...
The proxy cannot create unlimited capacity. All qualifying providers can be exhausted or unavailable together. In that case it reports failure rather than looping indefinitely or hiding failed output. The 256k floor excludes smaller and unknown limits and is never silently relaxed. Larger models are preferred within the same billing and health tier; this never enables paid billing or promotes an unhealthy route over a recently verified one. A paid switch is permission to use available paid access, not proof of funds or model access.
