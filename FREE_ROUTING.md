...
- Candidate order: saved billing categories first. Within free routes, the default family order is **DeepSeek V4.1 Flash → Kimi K3 → Qwen 3.8 Max → GLM 5.3 Flash**, across providers, then other eligible free models. Provider order and last success operate within each preference tier. Paid/subscription order is unchanged. Every new request returns to the highest available free preference after cooldown, without waiting on exhausted models. This is your configured preference, not a benchmarked ranking.
