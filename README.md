...
**Start here: [RNB setup, login recovery and automatic free routing](README_RNB.md).**

This is a hardened fork, not the upstream release. Use `Setup-Hardened.ps1` and `Claude-Free.ps1`. Do not use the upstream installer commands below. See [HARDENING.md](HARDENING.md), [VALIDATION.md](VALIDATION.md), and the retained [MIT license](LICENSE). “Secured” is a project suffix, not a zero-vulnerability guarantee.

---

## Original upstream documentation (reference only)
...
Reload VS Code or restart the app/IDE after connecting. In Codex, select an FCC
