...
AgentRouter can reject gateway clients with `401 unauthorized_client_error`. FCC displays that restriction and skips the rejected catalog. Ask AgentRouter to authorize gateway access or supply a supported endpoint; a saved or non-expiring key alone does not establish access. See the [reported restriction](https://github.com/agentrouter-org/docs/issues/21).
